...
def check_collisons(pipes):
    for pipe in pipes:
        if bird_rect.colliderect(pipe):
            death_sound.play()
            return False
    if bird_rect.top<=-50 or bird_rect.bottom>=440:
        return False
    return True   
...
def score_display(game_state):
    ...
    if game_state=='game_over':
        score_surface=game_font.render("Score:%s" %(int(score)),True,(255,255,255))
        score_rect=score_surface.get_rect(center=(120,50))
        screen.blit(score_surface,score_rect)
        
        high_score_surface=game_font.render("High Score:%s" %(int(high_score)),True,(255,255,255))
...
game_font=pygame.font.Font('04B_19.ttf',18)

#background surface
bg_surface=pygame.image.load('assets/background-day.png').convert()
# floor surface image 
floor_surface=pygame.image.load('assets/base.png').convert()
#moving floor positon
floor_x_pos=0
...
while True:
    ...
    if game_active:
        ...
        game_active=check_collisons(pipe_list)
        #pipes
